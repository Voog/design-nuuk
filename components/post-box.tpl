<article class="post_wrapper">
  <header class="post_header">
    ...
    <h3 class="post_title">{% if post-box == "article" %}{% editable article.title %}{% else %}<a href="{{ article.url }}">{{ article.title }}</a>{% endif %}</h3>
  </header>
  ...
  <div class="post_content">
    ...
    {% if post-box == "article" %}
      <div class="post_excerpt content-formatted" {{ edy_intro_edit_text }}>{% editable article.excerpt %}</div>
